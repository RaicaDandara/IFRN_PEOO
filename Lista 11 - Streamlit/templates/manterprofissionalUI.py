...
class ManterProfissionalUI:
    ...
    def inserir():
        ...
        email = st.text_input("Informe o email do profissional")
    ...
    def atualizar():
        ...
        if len(profissionais) == 0: 
            ...
            email = st.text_input("Informe o novo nome do profissional", op.email)
            espec = st.text_input("Informe a nova especialidade", op.especialidade)
            cons = st.text_input("Informe o novo conselho", op.conselho)
            senha = st.text_input("Informe a nova senha", op.senha)
            if st.button("Atualizar"):
                View.profissional_atualizar(op.id, nome, email, espec, cons, senha)
